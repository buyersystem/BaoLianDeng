//! tun2socks using netstack-smoltcp: reads raw IP packets from the macOS utun fd,
...
//!
//! macOS utun difference: each packet has a 4-byte AF header (AF_INET=2, AF_INET6=30)
//! that must be stripped on read and prepended on write.
...
async fn run_tun2socks(fd: RawFd, socks_addr: SocketAddr) -> io::Result<()> {
    ...
    // Task 4: TUN fd writer — prepend 4-byte AF header for macOS utun
    let tun_writer_handle = tokio::spawn(async move {
        while let Some(pkt) = egress_rx.recv().await {
            ...
            let af: u32 = if (pkt[0] >> 4) == 6 { 30 } else { 2 }; // AF_INET6 : AF_INET
            ...
            loop {
                let written =
                    unsafe { libc::write(fd, out.as_ptr() as *const c_void, out.len()) };
                if written >= 0 {
                    break;
                }
    ...
    // Task 5: TUN fd reader — strip 4-byte AF header, intercept DNS, feed TCP to stack
    ...
    let tun_reader_handle = tokio::spawn(async move {
        ...
        loop {
            ...
            loop {
                ...
                // Strip 4-byte utun AF header
                if n <= 4 {
                    continue;
                }
                let ip_data = &read_buf[4..n];

                // Intercept UDP DNS before sending to stack
                ...
                // Send to stack for TCP processing
...
async fn handle_dns_query(
    ...
    if let Some(response) = doh_client::resolve_via_doh(&query).await {
        ...
        // Build UDP reply: from DNS server (dst_ip, e.g. 198.18.0.2) back to
        // client (src_ip, e.g. 198.18.0.1). The TUN writer prepends the AF header.
        let raw = build_udp_packet(dst_ip, dst_port, src_ip, src_port, &response);
        let _ = reply_tx.send(raw);
    }
}
